...
// ---- Extract Fields (PHP 5.6 compatible) ----
...
$boilerType = isset($data['boilerType']) ? trim($data['boilerType']) : '';
...
if ($name === '' || $phone === '') {
    http_response_code(400);
    echo json_encode(array('error' => 'Name and phone are required'));
    exit;
}
...
if ($timestamp !== '' && is_numeric($timestamp)) {
    $dateStr = date('d.m.Y H:i:s', intval($timestamp / 1000));
}
...
<table style="border-collapse: collapse; width: 100%; max-width: 600px;">
    ...
    <tr style="background: #f5f5f5;">
        ...
        <td style="padding: 10px; border: 1px solid #ddd;">' . htmlspecialchars($region ? $region : '—') . '</td>
    </tr>';
...
if ($boilerType !== '') {
    if ($boilerType === 'steam') {
        $typeLabel = 'Паровой';
    } elseif ($boilerType === 'water') {
        $typeLabel = 'Водогрейный';
    } else {
        $typeLabel = htmlspecialchars($boilerType);
    }
    $html .= '
    <tr style="background: #f5f5f5;">
...
$html .= '
    <tr>
        ...
        <td style="padding: 10px; border: 1px solid #ddd;">' . htmlspecialchars($context ? $context : 'app') . '</td>
    </tr>
    <tr style="background: #f5f5f5;">
